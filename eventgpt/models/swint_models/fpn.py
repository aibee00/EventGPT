...
class FPN(nn.Module):
    ...
    def __init__(
        self, in_channels_list, out_channels, conv_block, top_blocks=None, drop_block=None, use_spp=False, use_pan=False,
            return_swint_feature_before_fusion=False, drop_first_feature=False
    ):
        """
        Arguments:
    ...
    def forward(self, x):
        ...
        # Convert tuple into tensor shape(bz, c, ndim)
        if self.drop_first_feature:
            results.pop(0)
        results = [t.view(t.shape[0], t.shape[1], -1) for t in results]
        results = [t.permute(0, 2, 1).contiguous() for t in results]  # tuple(shape(bz, ndim, c)), c=256,ndim=[28*28,14*14,7*7,4*4,2*2]
        # res_t = torch.concatenate(results, dim=-1)
        # return res_t.permute(0, 2, 1).contiguous()
