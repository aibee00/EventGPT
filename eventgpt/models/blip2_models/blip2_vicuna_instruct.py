...
# from eventgpt.models.blip2_models.quantization import quantize
...
class Blip2VicunaInstruct(Blip2Base):
    ...
    def __init__(
        ...
        from eventgpt.models.blip2_models.modeling_llama import LlamaForCausalLM
        ...
        # quantize 
        # self.llm_model.half()
        # self.llm_model = quantize(self.llm_model, 4)
        # self.llm_model = self.llm_model.quantize(4, empty_init=True) 
    ...
    def load_image_feature(self, image_path):
        """ Load image feature from disk

        The feature is numpy format, we need convert it to tensor 

        """
        # Find feature path according to image_path
        # feature path like: /ssd/wphu/Dataset/lavis/eventgpt/image_embeds/HONGQI-beijing-fkwd-20220109/ch01010_20220109181806.npy
        feature_path = image_path.replace('images', 'image_embeds').replace('.jpg', '.npy')
    ...
    def forward(self, samples):
        ...
        # print(f"samples image_path: {samples['image_path']}")
        image_embeds = []
        for image_path in samples["image_path"]:
            image_embeds.append(self.load_image_feature(image_path).to(image.device))
        ...
        text_output_tokens = self.llm_tokenizer(
            [t + self.llm_tokenizer.eos_token for t in samples['text_output']],  # answer
            return_tensors="pt",
            padding="longest",
            truncation=True,
