...
from eventgpt.processors.randaugment import RandomAugment
...
@registry.register_processor("box_caption_question")
class BoxCaptionQuestionProcessor(BaseProcessor):
    def __init__(self, max_words=50):
        self.max_words = max_words
...
@registry.register_processor("blip2_box_image_train")
class Blip2ImageTrainProcessor(BlipImageBaseProcessor):
    def __init__(
        self, image_size=364, mean=None, std=None, min_scale=0.5, max_scale=1.0
    ):
        ...
        self.transform = transforms.Compose(
            [
                transforms.Resize(
                    (image_size, image_size), interpolation=InterpolationMode.BICUBIC
                ),  # When use sam the other preprocess is putted in blip2_sam.py
                transforms.ToTensor(),
                self.normalize,
            ]
        )
...
@registry.register_processor("blip2_box_image_eval")
class BlipImageEvalProcessor(BlipImageBaseProcessor):
    def __init__(self, image_size=384, mean=None, std=None):
        super().__init__(mean=mean, std=std)
